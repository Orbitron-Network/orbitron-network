...
use orbitron_account_program::{OrbitronAccountType, ACCOUNT_LEN as ORBITRON_ACCOUNT_SIZE};
use orbitron_relying_party_program::RelyingPartyData;
...
impl JsonRpcRequestProcessor {
    ...
    fn get_orbitron_accounts_by_storage_key(
        &self,
        bank: &Arc<Bank>,
        storage_key: Pubkey,
        ...
        let is_target_orbitron_account = |account: &AccountSharedData| -> bool {
            *account.owner() == orbitron_account_program::id()
                && account.data().len() == ORBITRON_ACCOUNT_SIZE
                && matches!(
                    OrbitronAccountType::try_from(account.data()),
                    Ok(OrbitronAccountType::Account(_account_info))
                )
        };

        self.config
            .account_indexes
            .contains(&AccountIndex::OrbitronAccountStorage)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::OrbitronAccountStorage(storage_key),
                    is_target_orbitron_account,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
            })
    ...
    fn get_orbitron_accounts_by_owner_key(
        &self,
        bank: &Arc<Bank>,
        owner_key: Pubkey,
    ) -> Option<ScanResult<Vec<(Pubkey, AccountSharedData)>>> {
        let is_target_orbitron_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == orbitron_account_program::id()
                && match OrbitronAccountType::try_from(account.data()) {
                    Ok(OrbitronAccountType::Account(account)) => account.owners.contains(&owner_key),
                    _ => false,
                }
        };
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::OrbitronAccountOwner)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::OrbitronAccountOwner(owner_key),
                    is_target_orbitron_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
            })
    ...
    fn get_orbitron_relying_party_by_owner_key(
        &self,
        bank: &Arc<Bank>,
        owner_key: Pubkey,
    ) -> Option<ScanResult<Vec<(Pubkey, AccountSharedData)>>> {
        let is_target_orbitron_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == orbitron_relying_party_program::id()
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::OrbitronRelyingOwner)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::OrbitronRelyingOwner(owner_key),
                    is_target_orbitron_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
    ...
    fn get_orbitron_accounts_storages_by_operational_key(
        &self,
        bank: &Arc<Bank>,
        ...
        let is_target_orbitron_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == orbitron_account_program::id()
                && match OrbitronAccountType::try_from(account.data()) {
                    Ok(OrbitronAccountType::Storage(account_storage)) => account_storage
                        .operationals
                        .iter()
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::OrbitronAccountOperational)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::OrbitronAccountOperational(operational_key),
                    is_target_orbitron_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
...
pub mod rpc_minimal {
    ...
    pub trait Minimal {
        ...
        // TODO: Refactor `orbitron-validator wait-for-restart-window` to not require this method, so
...
pub mod rpc_accounts {
    ...
    pub trait AccountsData {
        ...
        #[rpc(meta, name = "getOrbitronAccountsByOperationalKey")]
        fn get_orbitron_accounts_by_operational_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ) -> Result<RpcResponse<Vec<String>>>;

        #[rpc(meta, name = "getOrbitronAccountsByOwnerKey")]
        fn get_orbitron_accounts_by_owner_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ...
        #[rpc(meta, name = "getOrbitronRelyingPartiesByOwnerKey")]
        fn get_orbitron_relying_parties_by_owner_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ) -> Result<RpcResponse<Vec<String>>>;
    ...
    impl AccountsData for AccountsDataImpl {
        ...
        fn get_orbitron_accounts_by_owner_key(
            ...
            debug!(
                "get_orbitron_accounts_by_owner_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::OrbitronAccountOwner);

            let owner_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let accounts = meta
                .get_orbitron_accounts_by_owner_key(&bank, owner_key)
                .transpose()
                .map_err(|e| RpcCustomError::ScanError {
                    message: e.to_string(),
                })?;
            debug!(
                "get_orbitron_accounts_by_owner_key orbitron accounts {:?}",
                accounts
            );
        ...
        fn get_orbitron_relying_parties_by_owner_key(
            ...
            debug!(
                "get_orbitron_relying_parties_by_owner_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::OrbitronRelyingOwner);

            let owner_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let accounts = meta
                .get_orbitron_relying_party_by_owner_key(&bank, owner_key)
                .transpose()
                .map_err(|e| RpcCustomError::ScanError {
                    message: e.to_string(),
                })?;
            debug!(
                "get_orbitron_relying_parties_by_owner_key orbitron accounts storages {:?}",
                accounts
            );
        ...
        fn get_orbitron_accounts_by_operational_key(
            ...
            debug!(
                "get_orbitron_accounts_by_operational_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::OrbitronAccountOperational);
            check_index!(&meta.config, AccountIndex::OrbitronAccountStorage);

            let operational_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let storages = meta
                .get_orbitron_accounts_storages_by_operational_key(&bank, operational_key)
                .transpose()
                .map_err(|e| RpcCustomError::ScanError {
                    message: e.to_string(),
                })?;
            debug!(
                "get_orbitron_accounts_by_operational_key orbitron accounts storages {:?}",
                storages
            );

            let accounts: std::result::Result<Vec<_>, _> = storages
                .expect("orbitron account operational index to be activated")
                .into_iter()
                .map(|(storage, _)| {
                    meta.get_orbitron_accounts_by_storage_key(&bank, storage)
                        .expect("orbitron account storage index to be activated")
                })
                .collect();
            ...
            debug!(
                "get_orbitron_accounts_by_operational_key orbitron accounts {:?}",
                accounts
            );
