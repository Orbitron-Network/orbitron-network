...
impl<D: OrbitronDBCommon> Clone for Storage<D> {
    fn clone(&self) -> Self {
        Self {
...
impl<D: OrbitronDBCommon> std::fmt::Debug for Storage<D> {
...
static SECONDARY_MODE_PATH_SUFFIX: &str = "orbitron-secondary";
...
pub trait OrbitronDBCommon: DBAccess + std::fmt::Debug + Sized {
...
impl OrbitronDBCommon for rocksdb::DBWithThreadMode<rocksdb::SingleThreaded> {
    fn flush(&self) -> Result<(), rocksdb::Error> {
        self.flush()
    }
...
impl OrbitronDBCommon for OptimisticTransactionDB {
    fn flush(&self) -> Result<(), rocksdb::Error> {
        self.flush()
    }
...
pub struct DbWithClose<D: OrbitronDBCommon>(D);
...
impl<D: OrbitronDBCommon> std::fmt::Debug for DbWithClose<D> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
