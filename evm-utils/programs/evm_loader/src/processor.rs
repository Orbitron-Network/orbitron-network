...
impl EvmProcessor {
    pub fn process_instruction(
        ...
        let cross_execution_enabled = invoke_context
            .feature_set
            .is_active(&solana_sdk::feature_set::orbitron::evm_cross_execution::id());
        let register_swap_tx_in_evm = invoke_context
            .feature_set
            .is_active(&solana_sdk::feature_set::orbitron::native_swap_in_evm_history::id());
        ...
        let ignore_reset_on_cleared = invoke_context
            .feature_set
            .is_active(&solana_sdk::feature_set::orbitron::ignore_reset_on_cleared::id());
        let free_ownership_require_signer = invoke_context
            .feature_set
            .is_active(&solana_sdk::feature_set::orbitron::free_ownership_require_signer::id());
        let borsh_serialization_enabled = invoke_context
            .feature_set
            .is_active(&solana_sdk::feature_set::orbitron::evm_instruction_borsh_serialization::id());
        ...
        if register_swap_tx_in_evm {
            executor.reset_balance(*precompiles::ETH_TO_OBTT_ADDR, ignore_reset_on_cleared)
        }
    ...
    fn process_execute_tx(
        ...
        fn precompile_set(
            ...
            match (support_precompile, evm_new_precompiles) {
                ...
                (true, false) => precompiles::PrecompileSet::OrbitronClassic,
                (true, true) => precompiles::PrecompileSet::OrbitronNext,
            }
        }
        ...
        let result = match tx {
            ExecuteTransaction::Signed { tx } => {
                ...
                let activate_precompile = precompile_set(
                    ...
                    invoke_context
                        .feature_set
                        .is_active(&solana_sdk::feature_set::orbitron::evm_new_precompiles::id()),
                );
    ...
    fn process_swap_to_evm(
        ...
        if register_swap_tx_in_evm {
            executor.register_swap_tx_in_evm(*precompiles::ETH_TO_OBTT_ADDR, evm_address, gweis)
        }
        Ok(())
    }
...
mod test {
        ...
        fn process_transaction(&mut self, ixs: Vec<Instruction>) -> Result<(), InstructionError> {
            ...
            let evm_executor = evm_state::Executor::with_config(
                ...
                evm::EvmConfig::new(
                    evm::TEST_CHAIN_ID,
                    self.feature_set
                        .is_active(&solana_sdk::feature_set::orbitron::burn_fee::id()),
                ),
                evm_state::executor::FeatureSet::new(
                    ...
                    self.feature_set.is_active(
                        &solana_sdk::feature_set::orbitron::accept_zero_gas_price_with_native_fee::id(
                        ),
                    ),
                ),
            );
            ...
            for instruction in ixs {
                ...
                if let Err(e) = invoke_context.process_instruction(
                    ...
                    let clear_logs = self.feature_set.is_active(
                        &solana_sdk::feature_set::orbitron::clear_logs_on_native_error::id(),
                    );
                    self.evm_state
    ...
    fn execute_tx() {
        ...
        evm_context.disable_feature(&solana_sdk::feature_set::orbitron::burn_fee::id());
        let secret_key = evm::SecretKey::from_slice(&SECRET_KEY_DUMMY).unwrap();
    ...
    fn execute_transfer_to_native_without_needed_account() {
        ...
        let tx_call = evm::UnsignedTransaction {
            ...
            gas_price: 1u32.into(),
            gas_limit: 300000u32.into(),
            action: TransactionAction::Call(*precompiles::ETH_TO_OBTT_ADDR),
            value: crate::scope::evm::lamports_to_gwei(lamports_to_send_back),
            input: precompiles::ETH_TO_OBTT_CODE
                .abi
        ...
        let tx_call = tx_call.sign(&ether_sc, Some(CHAIN_ID));
        {
            ...
            assert_eq!(
                evm_context
                    .evm_state
                    .get_account_state(*precompiles::ETH_TO_OBTT_ADDR)
                    .unwrap()
                    .balance,
                0u32.into()
            )
    ...
    fn test_revert_clears_logs() {
...

        {
            ...
            // Reverted tx with clear_logs_on_error disabled don't clear logs
            {
                let mut evm_context = evm_context.clone(); // make copy for test
                evm_context
                    .disable_feature(&solana_sdk::feature_set::orbitron::clear_logs_on_error::id());
