...
pub static ETH_TO_OBTT_ADDR: Lazy<H160> = Lazy::new(|| {
    H160::from_str(concat!(
        "56454c41532d434841494e", // 'Orbitron-CHAIN'
...
pub static ETH_TO_OBTT_CODE: Lazy<NativeContract<EthToVlxImp, Pubkey>> = Lazy::new(|| {
    #[allow(deprecated)]
    let abi = Function {
    ...
    fn implementation(
        ...
        log::trace!("Precompile ETH_TO_OBTT");

        if !matches!(
            cx.precompile_context.call_scheme,
            None | Some(CallScheme::Call)
        ) || cx.precompile_context.evm_context.address != *ETH_TO_OBTT_ADDR
    ...
    fn handle_promise(accounts: AccountStructure, promise: EthToVlxResult) -> Result<()> {
        log::trace!("Promise handle ETH_TO_OBTT {:?}", promise);
