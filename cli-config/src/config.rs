...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "orbitron", "cli", "config.yml"]);
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend([".config", "orbitron", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://api.mainnet.orbitron.network".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://api.devnet.orbitron.network"),
            "ws://api.devnet.orbitron.network/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://api.devnet.orbitron.network"),
            "wss://api.devnet.orbitron.network/".to_string()
        );
