...
RELEASE_BASENAME="${RELEASE_BASENAME:=orbitron-release}"
...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/orbitron-install-init orbitron-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml orbitron-install-init-"$TARGET"* $MAYBE_TARBALLS; do
