...
use orbitron_account_program::{VAccountStorage, OrbitronAccountType};
...
pub enum IndexKey {
    ...
    OrbitronAccountStorage(Pubkey),
    OrbitronAccountOwner(Pubkey),
    OrbitronAccountOperational(Pubkey),
    OrbitronRelyingOwner(Pubkey),
}
...
pub enum AccountIndex {
    ...
    SplTokenMint,
    SplTokenOwner,
    OrbitronAccountStorage,
    OrbitronAccountOwner,
    OrbitronAccountOperational,
    OrbitronRelyingOwner,
}
...
impl AccountIndex {
    pub fn name(&self) -> &str {
        match self {
            ...
            AccountIndex::OrbitronAccountStorage => "Orbitron Account Storage",
            AccountIndex::OrbitronAccountOwner => "Orbitron Account Owner",
            AccountIndex::OrbitronAccountOperational => "Orbitron Account Operational",
            AccountIndex::OrbitronRelyingOwner => "Orbitron Relying Owner",
        }
    }
}
...
pub struct AccountsIndex<T: IndexValue> {
    ...
    // Orbitron Indices
    orbitron_account_storage_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    orbitron_account_owner_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    orbitron_account_operational_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    orbitron_relying_party_owner_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
...
impl<T: IndexValue> AccountsIndex<T> {
    ...
    pub fn new(config: Option<AccountsIndexConfig>) -> Self {
        ...
        Self {
            ...
            //Orbitron indexes
            orbitron_account_storage_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("orbitron_account_storage_index"),
            orbitron_account_owner_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("orbitron_account_owner_index"),
            orbitron_account_operational_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("orbitron_account_operational_index"),
            orbitron_relying_party_owner_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("orbitron_relying_party_owner_index"),
        }
    }
        ...
        R: RangeBounds<Pubkey> + std::fmt::Debug,
    {
        ...
        */
        match scan_type {
            ...
            ScanTypes::Indexed(IndexKey::OrbitronAccountStorage(va_storage_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.orbitron_account_storage_index,
                    &va_storage_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::OrbitronAccountOwner(va_owner_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.orbitron_account_owner_index,
                    &va_owner_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::OrbitronAccountOperational(va_operational_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.orbitron_account_operational_index,
                    &va_operational_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::OrbitronRelyingOwner(va_owner_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.orbitron_relying_party_owner_index,
                    &va_owner_key,
                    Some(max_root),
                    config,
                ),
    ...
    pub(crate) fn update_secondary_indexes(
        ...
        // Orbitron account
        if *account_owner == orbitron_account_program::id() {
            match OrbitronAccountType::try_from(account_data) {
                Ok(OrbitronAccountType::Account(account_info)) => {
                    if account_indexes.contains(&AccountIndex::OrbitronAccountStorage) {
                        let storage = account_info.find_storage_key(pubkey);
                        self.orbitron_account_storage_index.insert(&storage, pubkey);
                    }

                    if account_indexes.contains(&AccountIndex::OrbitronAccountOwner) {
                        for owner in account_info.owners {
                            if owner != Pubkey::default() {
                                self.orbitron_account_owner_index.insert(&owner, pubkey);
                            }
                        }
                    }
                }
                Ok(OrbitronAccountType::Storage(VAccountStorage { operationals })) => {
                    if account_indexes.contains(&AccountIndex::OrbitronAccountOperational) {
                ...
                Err(err) => log::warn!("Unable to parse Orbitron Account: {:?}", err),
            }
        }

        // Orbitron relying party data
        if *account_owner == orbitron_relying_party_program::id() {
            match RelyingPartyData::try_from(account_data) {
                Ok(acc) => {
                    if account_indexes.contains(&AccountIndex::OrbitronRelyingOwner) {
                        self.orbitron_relying_party_owner_index
