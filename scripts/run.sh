...
ok=true
for program in orbitron-{faucet,genesis,keygen,validator}; do
    $program -V || ok=false
done
$ok || {
    echo
...
if [[ -e $validator_vote_account ]]; then
    ...
    orbitron-keygen new --no-passphrase -so "$validator_vote_account"
fi
...
if [[ -e $validator_stake_account ]]; then
    ...
    orbitron-keygen new --no-passphrase -so "$validator_stake_account"
fi
...
args=(
    ...
    --account-index orbitron-accounts-storages
    --account-index orbitron-accounts-owners
    --account-index orbitron-accounts-operationals
    --evm-state-archive "$ledgerDir"/archive-evm
)
# shellcheck disable=SC2086
orbitron-validator "${args[@]}" $SOLANA_RUN_SH_VALIDATOR_ARGS &
validator=$!
